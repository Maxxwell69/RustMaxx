...
**Same anchor without RustMaxx URL fields:** set **`DefaultAnchorSteamId`** in **`oxide/config/MaxxInvaders.json`**, **MAXX SETTINGS** (field is under **Streamer HUD**; **1.6.6+** moved it so it stays on-screen), or chat **`/maxxinvaders anchor <17-digit Steam64>`** (admin). Then RCON `maxxinvaders.spawn` **without** the 7th argument still uses you as spawn ring + RoamingNPCs bridge anchor — matching **GUI** spawns.
