...
4. If `TWITCH_WEBHOOK_CALLBACK_URL` and `TWITCH_EVENTSUB_SECRET` are set, the backend creates EventSub subscriptions: `channel.follow` uses your (broadcaster) token (scope `channel:read:subscriptions`); chat uses the app token.
