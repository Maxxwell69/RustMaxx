...
| `streamer_medic` | **Field medic** for MaxxInvaders/TikFinity anchor bots: surgeon scrubs, revolver + ammo, medical syringes + large medkits. **RoamingNPCs 0.5.51+** auto-adds this template on load (revive/heal anchor when wounded or below ~50% HP — server-side heal/revive). Spawn with **`?template=streamer_medic`** or set MaxxInvaders **`ViewerRoamingTemplateKey`** / env to **`streamer_medic`**. |
