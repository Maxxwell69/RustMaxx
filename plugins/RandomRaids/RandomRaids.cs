...
namespace Oxide.Plugins
{
    ...
    public class RandomRaids : RustPlugin
    {
        ...
        private const string ignoreperm = "randomraids.ignore";
        ...
        private void Init()
        {
            ...
            EnsureChaosRaidProfiles();
        }

        /// <summary>RustChaos chaosraid_* presets: progressive NPC waves + juggernaut bosses; rewards match cloned base tiers.</summary>
        private void EnsureChaosRaidProfiles()
        {
            try
            {
                bool dirty = false;
                if (!configData.juggernautProfile.ContainsKey("chaos_easy_boss"))
                {
                    configData.juggernautProfile.Add("chaos_easy_boss",
                        new juggernautConfig(new Dictionary<int, int> { { 4, 1 } }, 550f, new List<string>()));
                    dirty = true;
                }

                if (!configData.juggernautProfile.ContainsKey("chaos_medium_boss"))
                {
                    configData.juggernautProfile.Add("chaos_medium_boss",
                        new juggernautConfig(new Dictionary<int, int> { { 8, 2 } }, 650f, new List<string>()));
                    dirty = true;
                }

                if (!configData.juggernautProfile.ContainsKey("chaos_hard_boss"))
                {
                    configData.juggernautProfile.Add("chaos_hard_boss",
                        new juggernautConfig(new Dictionary<int, int> { { 10, 1 } }, 900f, new List<string>()));
                    dirty = true;
                }

                if (!configData.raidSettings.raidTypes.ContainsKey("easy") ||
                    !configData.raidSettings.raidTypes.ContainsKey("medium") ||
                    !configData.raidSettings.raidTypes.ContainsKey("hard"))
                    return;

                if (!configData.raidSettings.raidTypes.ContainsKey("chaos_easy"))
                {
                    var json = JsonConvert.SerializeObject(configData.raidSettings.raidTypes["easy"]);
                    var c = JsonConvert.DeserializeObject<raidConfig>(json);
                    c.TotalNpcWaves = 3;
                    c.TotalNpcs = 10;
                    c.PerWaveNpcCounts = new List<int> { 1, 2, 3, 4 };
                    c.NextWaveSeconds = 120;
                    c.RunTimeSeconds = 900;
                    c.juggernaut = new List<string> { "chaos_easy_boss" };
                    configData.raidSettings.raidTypes["chaos_easy"] = c;
                    dirty = true;
                }

                if (!configData.raidSettings.raidTypes.ContainsKey("chaos_medium"))
                {
                    var json = JsonConvert.SerializeObject(configData.raidSettings.raidTypes["medium"]);
                    var c = JsonConvert.DeserializeObject<raidConfig>(json);
                    c.TotalAuth = 1;
                    c.blockMIN = 12;
                    c.TotalNpcWaves = 7;
                    c.TotalNpcs = 15;
                    c.PerWaveNpcCounts = new List<int> { 1, 2, 3, 4, 5, 6, 7, 8 };
                    c.NextWaveSeconds = 120;
                    c.RunTimeSeconds = 2400;
                    c.attackCopter = false;
                    c.juggernaut = new List<string> { "chaos_medium_boss" };
                    configData.raidSettings.raidTypes["chaos_medium"] = c;
                    dirty = true;
                }

                if (!configData.raidSettings.raidTypes.ContainsKey("chaos_hard"))
                {
                    var json = JsonConvert.SerializeObject(configData.raidSettings.raidTypes["hard"]);
                    var c = JsonConvert.DeserializeObject<raidConfig>(json);
                    c.TotalAuth = 1;
                    c.blockMIN = 16;
                    c.TotalNpcWaves = 9;
                    ...
                    for (int i = 1; i <= 10; i++) counts.Add(i);
                    c.PerWaveNpcCounts = counts;
                    c.NextWaveSeconds = 120;
                    c.RunTimeSeconds = 4200;
                    c.attackCopter = true;
                    c.juggernaut = new List<string> { "chaos_hard_boss" };
                    configData.raidSettings.raidTypes["chaos_hard"] = c;
                    dirty = true;
                }
            ...
            {
                PrintWarning($"EnsureChaosRaidProfiles failed: {ex.Message}");
            }
        }

        private static int ResolveNpcCountForWave(raidConfig cfg, int currentWaveAfterIncrement)
        {
            if (cfg == null || currentWaveAfterIncrement < 1) return 1;
            if (cfg.PerWaveNpcCounts != null && cfg.PerWaveNpcCounts.Count > 0 &&
                currentWaveAfterIncrement <= cfg.PerWaveNpcCounts.Count)
                return Mathf.Max(1, cfg.PerWaveNpcCounts[currentWaveAfterIncrement - 1]);
            return Mathf.Max(1, cfg.TotalNpcs);
        }
        ...
        private void OnServerInitialized()
        {
            ...
            foreach (BuildingPrivlidge find in BaseNetworkable.serverEntities.OfType<BuildingPrivlidge>())
            {
                ...
                {
                    if (!pcdData.tcData.ContainsKey(playerNameID))
                        pcdData.tcData.Add(playerNameID, new List<ulong>() { find.net.ID.Value });
                    else if (!pcdData.tcData[playerNameID].Contains(find.net.ID.Value))
                        pcdData.tcData[playerNameID].Add(find.net.ID.Value);
                }
            }
        ...
        {
            pcdData.tcData = new Dictionary<ulong, List<ulong>>();
            pcdData.cooldown = new Dictionary<ulong, float>();
            raidData.data = new Dictionary<ulong, rData>();
            logData.pLogs = new Dictionary<ulong, lData>();
            logDataRB.pLogsRB = new Dictionary<ulong, lDataRB>();
            SaveData(fileName);
            SaveLogData();
            SaveRaidData();
            SaveRBLogData();
            PrintWarning("New map detected wiping all data!");
        }
        ...
        public class PlayerEntity
        {
            ...
            public Dictionary<ulong, float> cooldown = new Dictionary<ulong, float>();
        }
        ...
        public void SaveData(string filename = "BQVk")
        {
        ...
        public class raidConfig
        {
            ...
            /// <summary>Optional per-wave NPC counts (wave 1 = index 0). When set, overrides Total npcs per wave for matching waves.</summary>
            [JsonProperty(PropertyName = "NPC counts per wave (optional; overrides Total npcs per wave — length must equal total waves)")]
            public List<int> PerWaveNpcCounts { get; set; }
        ...
        public class EventRandomManager : FacepunchBehaviour
        {
            ...
            {
                return;//removed untell FP adds back in assets they took away from develipers.
            ...
            public IEnumerator GenerateEventMembers()
            {
                ...
                newWave = false;
                int npcThisWave = ResolveNpcCountForWave(config, currentWave);
                int totalNpc = 0;
                ...
                int denom = Mathf.Max(1, npcThisWave);
                List<Vector3> positions = generateSpawnList(location, spawnDictance, radius / denom);

                if (savedPosition.Count <= 0)
                    savedPosition = new List<Vector3>(positions);

                yield return CoroutineEx.waitForSeconds(5f);
                while (positions.Count < npcThisWave)
                {
                    if (radius < 50 || (radius / denom) <= 0)
                    {
            ...
            public static object FindPointOnNavmesh(Vector3 targetPosition, float maxDistance = 4f)
            {
                for (int i = 0; i < 10; i++)
                {
                    ...
                    if (NavMesh.SamplePosition(position, out navmeshHit, maxDistance, 1))
                    {
                        ...
                        if (IsNearWorldCollider(navmeshHit.position) || IsOn(navmeshHit.position))
                            continue;
            ...
            internal static bool IsOn(Vector3 point) => Physics.Raycast(point, Vector3.down, out _hit, 3f, Layers.Mask.World, QueryTriggerInteraction.Ignore) && IsRock(_hit.collider.name);
            internal static bool IsRock(string name) => _prefabs.Exists(value => name.Contains(value, CompareOptions.OrdinalIgnoreCase));
            internal static List<string> _prefabs = new List<string> { "rock", "formation", "cliff", "prevent_movement", "prevent", "ZGZmZmR=" };
        ...
        public class RandomRaider : NPCPlayer, IAISenses, IAIAttack, IThinker
        {
            ...
            public void setUpGear(string kitname = "", string ignore = "BQVk")
            {
        ...
        public class RandomRaiderBrain : BaseAIBrain
        {
            ...
            public class CombatState : BasicAIState
            {
                ...
                public override StateStatus StateThink(float delta, BaseAIBrain brain, BaseEntity entity)
                {
                    ...
                    if (!reloading && entityAI != null && entityAI.IsReloading())
                    {
                        reloading = true;
                        entityAI.SignalBroadcast(BaseEntity.Signal.Reload);
                    }
                    else if (reloading && !entityAI.IsReloading())
                        reloading = false;
                    ...
                    else if (entityAI.currentTarget != null)
                    {
                        ...
                        if (entityAI.InEngagementRange() && !entityAI.IsReloading())
        ...
        private void OnRaidableBaseCompleted(Vector3 raidPos, int mode, bool allowPVP, string id, float spawnTime, float despawnTime, float loadTime, ulong ownerId, BasePlayer owner, List<BasePlayer> raiders, List<BasePlayer> intruders, List<BaseEntity> entities)
        {
            ...
            List<ulong> authedTemp = Pool.Get<List<ulong>>();
                ...
                if (raiders.Count > 0 && configData.settingsRB.raidSettings.ContainsKey(mode))
                {
                    ...
                    if (rando <= config.raidChance && pcdData.tcData.Count > 0)
                    {
                        ...
                        foreach (var p in raiders)
                        {
                            ...
                            if (permission.UserHasPermission(p.UserIDString, ignoreperm))
                            {
                                if (debug) PrintWarning($"Testing Players Permisions in the raid failed Player: {p.displayName} has the ignore permision.");
                                continue;
                            }
                            ...
                            if (!string.IsNullOrEmpty(type))
                            {
                                if (configData.settingsRB.warnPlayer)
                                {
                                    ...
                                    {
                                        if (!authPlayers.Contains(user))
                                            authPlayers.Add(user);
                                        BasePlayer TCplayer = BasePlayer.FindByID(user);
                                        if (TCplayer != null && TCplayer.IsConnected)
                                        {
                                            ...
                                            if (!raidData.data.ContainsKey(user))
                                                raidData.data.Add(user, new rData() { LastRaidedRB = DateTime.Now.AddMinutes(config.cooldownRB) });
                                            else raidData.data[user].LastRaidedRB = DateTime.Now.AddMinutes(config.cooldownRB);
                                        }
                                    }
                                }
        ...
        public IEnumerator FindRandomBaseToRaid(bool offline = false, bool useChance = true, BasePlayer player = null)
        {
            ...
            {
                if (configData.settings.musthavePerm && !permission.UserHasPermission(p.UserIDString, usePerm) || permission.UserHasPermission(p.UserIDString, ignoreperm))
                    continue;
                    ...
                    foreach (ulong key in pcdData.tcData[p.userID.Get()].ToList())
                    {
                        ...
                        else if (priv == null)
                        {
                            ...
                            if (availableTypes.Count > 0)
                            {
                                ...
                                {
                                    if (!raidData.data.ContainsKey(user))
                                        raidData.data.Add(user, new rData() { LastRaided = DateTime.Now });
                                    else raidData.data[user].LastRaided = DateTime.Now;
        ...
        private bool CanBeRaided(BuildingPrivlidge priv)
        {
            ...
            if (priv != null)
            {
                ...
                foreach (var user in priv.authorizedPlayers)
                {
                    if (raidData.data.ContainsKey(user))
                    {
                        if (raidData.data[user].LastRaided.AddMinutes(configData.settings.raidAgainCooldown) > DateTime.Now)
                        {
                            return false;
                        }

                        if (raidData.data[user].LastRaidedNpcKills > DateTime.Now)
                        {
                            return false;
                        }

                        if (raidData.data[user].LastRaidedRB > DateTime.Now)
                        {
                            return false;
                        }
                    }
        ...
        private object OnCupboardClearList(BuildingPrivlidge privilege, BasePlayer player)
        {
            ...
            {
                 if (pcdData.tcData.ContainsKey(playerNameID) && pcdData.tcData[playerNameID].Contains(privilege.net.ID.Value))
                    pcdData.tcData[playerNameID].Remove(privilege.net.ID.Value);
            }
            SaveData();
            return null;
        }
        ...
        {
            if (player == null || info == null || info.InitiatorPlayer == null || info.InitiatorPlayer.userID == null || !info.InitiatorPlayer.userID.IsSteamId() || player is RandomRaider || player.skinID == 14922524) return;           
            string name = player.ShortPrefabName;

            if (!string.IsNullOrEmpty(name))
            {
                ulong usersID = info.InitiatorPlayer.userID.Get();
                ...
                if (!logData.pLogs.ContainsKey(usersID))
                    logData.pLogs.Add(usersID, new lData());

                if (configData.settingsNPCKILLS.enableKillsCombine && configData.settingsNPCKILLS.CombineNpcList.Contains(name))
                {
                    logData.pLogs[usersID]._totalNpcKills++;
                    SaveLogData();
                    if (logData.pLogs[usersID]._totalNpcKills >= configData.settingsNPCKILLS.CombineProfile.totalRaids)
                    {
                        ...
                        if (configData.settingsNPCKILLS.CombineProfile.resetKillCount)
                            logData.pLogs[usersID]._totalNpcKills = 0;
                    }
                    return;
                }
                ...
                { 
                    if (!logData.pLogs[usersID].NPCKILLS.ContainsKey(name))
                        logData.pLogs[usersID].NPCKILLS.Add(name, 1);
                    else
                        logData.pLogs[usersID].NPCKILLS[name]++;
                    SaveLogData();
                    if (logData.pLogs[usersID].NPCKILLS[name] >= configData.settingsNPCKILLS.npcSettings[name].totalRaids)
                    { 
                ...
                if (rando <= config.raidChance && pcdData.tcData.Count > 0)
                {
                    ...
                    if (!string.IsNullOrEmpty(type))
                    {
                        foreach (var user in priv.authorizedPlayers)
                        {
                            ...
                            {
                                if (logData.pLogs.ContainsKey(user) && logData.pLogs[user].NPCKILLS != null)
                                    logData.pLogs[user].NPCKILLS.Clear();  
                                SaveLogData();
                            ...
                            {
                                BasePlayer p2 = BasePlayer.FindByID(user);
                                if (p2 != null)
                                {
        ...
        /// <summary>RustChaos webhook: same rules as admin <see cref="CallRaidHere"/> but no chat replies — returns true or an error token string.</summary>
        [HookMethod("TryStartRaidForStreamer")]
        public object ApiTryStartRaidForStreamer(BasePlayer player, string raidTypeKey)
        {
            if (player == null || !player.IsConnected) return "no_player";
            if (string.IsNullOrWhiteSpace(raidTypeKey)) return "invalid_type";

            raidTypeKey = raidTypeKey.Trim();
            if (!configData.raidSettings.raidTypes.ContainsKey(raidTypeKey))
                return "unknown_raid_type";

            BuildingPrivlidge privFind = player.GetBuildingPrivilege();
            if (privFind == null || !privFind.IsAuthed(player))
                return "need_building_privilege";
            ...
            BuildingManager.Building build = privFind.GetBuilding();
            if (build == null || !build.HasBuildingBlocks() || build.buildingBlocks.Count < 5)
                return "base_too_small";

            var block = GenerateTargets(privFind);
            if (block.Count < 5)
                return "base_too_small";
            ...
            startRandomRaid(privFind.transform.position, authPlayers, raidTypeKey, block);
            return true;
        }
        #endregion
